<script>
	...
	let filteredMarkets = $derived(
		markets.filter((m) => {
			...
			if (
				selectedAsset !== 'all' &&
				!m.underlying.toLowerCase().includes(selectedAsset.toLowerCase())
			)
				return false;
			if (
				searchQuery &&
...
<svelte:head>
	...
	<meta
		name="description"
		content="Trade PT and YT tokens. Earn fixed yields or speculate on yield movements."
	/>
</svelte:head>

<div class="min-h-screen bg-[#0a0b0f] font-sans text-white">
	<!-- Ambient Background -->
	<div class="pointer-events-none fixed inset-0 overflow-hidden">
		<div
			class="absolute -top-40 right-1/4 h-[600px] w-[600px] rounded-full bg-violet-500/5 blur-[180px]"
		></div>
		<div
			class="absolute top-1/3 -left-20 h-[400px] w-[400px] rounded-full bg-purple-500/5 blur-[120px]"
		></div>
	</div>
	...
	<div class="relative mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
		...
		<div class="mb-8 flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
			<div>
				<div class="mb-2 flex items-center gap-3">
					<div
						class="flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br from-violet-500 to-purple-600 text-xl shadow-lg shadow-violet-500/20"
					>
						⚡
					</div>
					<div class="flex items-center gap-2">
						<span class="text-xs font-black tracking-[0.3em] text-violet-400 uppercase"
							>JamCat DeFi</span
						>
						<span
							class="flex h-2 w-2 animate-pulse rounded-full bg-green-400 shadow-[0_0_8px_rgba(74,222,128,0.8)]"
						></span>
					</div>
				</div>
				<h1 class="text-4xl font-black tracking-tight sm:text-5xl lg:text-6xl">
					<span
						class="bg-gradient-to-r from-white via-violet-200 to-purple-400 bg-clip-text text-transparent"
					>
						PendleFi
					</span>
				</h1>
				<p class="mt-2 max-w-lg text-sm text-slate-400">
					Yield tokenization protocol. Split yield-bearing assets into Principal Tokens (PT) and
					Yield Tokens (YT). Earn fixed yields or trade yield speculation.
				</p>
			</div>
			...
			<div class="flex flex-wrap items-center gap-2">
				<button
					...
					class="rounded-full border px-4 py-2 text-xs font-bold transition-all {viewMode === 'grid'
						? 'border-violet-500/50 bg-violet-500/20 text-white shadow-[0_0_15px_rgba(139,92,246,0.3)]'
				...
				<button
					...
					class="rounded-full border px-4 py-2 text-xs font-bold transition-all {viewMode === 'list'
						? 'border-violet-500/50 bg-violet-500/20 text-white shadow-[0_0_15px_rgba(139,92,246,0.3)]'
						: 'border-white/10 bg-white/5 text-slate-400 hover:border-white/20 hover:bg-white/10'}"
				>
					☰ List
		...
		<div class="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
			<div class="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-4 backdrop-blur-sm">
				<div
					class="mb-1 flex items-center gap-1.5 text-[10px] font-bold tracking-wide text-slate-500 uppercase"
				>
		...
		<div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
			<div class="flex flex-wrap items-center gap-2">
				<span class="text-xs font-bold tracking-wide text-slate-500 uppercase">Chain:</span>
				{#each chains as chain}
					<button
						onclick={() => (selectedChain = chain.id)}
						class="rounded-full border px-3 py-1.5 text-xs font-bold transition-all {selectedChain ===
						chain.id
			...
			<div class="flex items-center gap-2">
				<span class="text-xs font-bold tracking-wide text-slate-500 uppercase">Asset:</span>
				<select
					bind:value={selectedAsset}
					class="rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-xs font-bold text-white focus:border-violet-500/50 focus:outline-none"
				>
		...
		<div class="mb-6">
			<div class="relative max-w-md">
				<span class="absolute top-1/2 left-3 -translate-y-1/2 text-slate-500">🔍</span>
				<input
					bind:value={searchQuery}
					placeholder="Search PT markets..."
					class="w-full rounded-xl border border-white/10 bg-white/5 py-2.5 pr-4 pl-10 text-sm text-white placeholder-slate-500 focus:border-violet-500/50 focus:shadow-[0_0_15px_rgba(139,92,246,0.1)] focus:outline-none"
				/>
			</div>
		</div>
		...
		{#if loading}
			<div class="flex h-64 flex-col items-center justify-center gap-4">
				<div class="relative">
					<div
						class="h-14 w-14 animate-spin rounded-full border-4 border-slate-800 border-t-violet-500"
					></div>
					<div
						class="absolute inset-0 h-14 w-14 animate-pulse rounded-full border-4 border-violet-500/30"
					></div>
				</div>
		...
		{:else if error}
			<div class="rounded-2xl border border-red-500/20 bg-red-500/10 p-12 text-center">
				<div
					class="mb-4 inline-flex h-16 w-16 items-center justify-center rounded-2xl bg-red-500/20 text-3xl"
				>
					⚠️
				</div>
				...
				<button
					onclick={fetchMarkets}
					class="mt-6 rounded-xl border border-red-500/30 bg-red-500/10 px-6 py-2.5 text-sm font-bold text-red-400 transition hover:bg-red-500/20"
					>↻ Retry</button
				>
			</div>
		...
		{:else if viewMode === 'grid'}
			<div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
				{#each filteredMarkets as market, i (market.id)}
					...
					<div
						class="group relative overflow-hidden rounded-2xl border border-white/[0.06] bg-white/[0.02] p-5 transition-all duration-300 hover:-translate-y-1 hover:border-violet-500/30 hover:bg-white/[0.04] hover:shadow-lg hover:shadow-violet-500/10"
						style="animation:fadeUp 0.4s ease {i * 50}ms backwards"
					>
						<!-- Glow -->
						<div
							class="absolute -top-10 -right-10 h-32 w-32 rounded-full bg-gradient-to-br {colors.from} to-transparent opacity-0 blur-[60px] transition-opacity duration-500 group-hover:opacity-100"
						></div>
						...
						<div class="relative mb-4 flex items-start justify-between">
							<div class="flex items-center gap-3">
								<div
									class="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br {colors.from} {colors.to} text-sm font-bold text-white shadow-lg"
								>
							...
							<span
								class="rounded-full border border-white/10 bg-white/5 px-2 py-0.5 text-[10px] font-bold text-slate-400"
							>
						...
						<!-- Expand Button -->
						<button
							onclick={() => (expandedMarket = isExpanded ? null : market.id)}
						...
						{#if isExpanded}
							<div
								class="relative mt-4 space-y-3 border-t border-white/[0.06] pt-4"
								style="animation:expand 0.3s ease"
							>
								<div class="grid grid-cols-2 gap-2 text-xs">
									...
									<div class="flex justify-between">
										...
										<span class="font-bold text-emerald-400"
											>{((1 - market.ptPrice) * 100).toFixed(2)}%</span
										>
									</div>
								</div>
								<a
									href={`https://app.pendle.finance/trade/markets/${market.chain}/${market.id}`}
									target="_blank"
									rel="noopener"
		...
		{:else}
			...
			<div class="overflow-hidden rounded-2xl border border-white/[0.06] bg-white/[0.02]">
				<div
					class="grid grid-cols-[2fr_1fr_1fr_1fr_1fr_1.5fr] items-center gap-4 border-b border-white/[0.06] px-4 py-3 text-[10px] font-bold tracking-wider text-slate-500 uppercase"
				>
					<span>Market</span>
				...
				{#each filteredMarkets as market, i (market.id)}
					{@const colors = getMarketColor(market.name)}
					<div
						class="grid grid-cols-[2fr_1fr_1fr_1fr_1fr_1.5fr] items-center gap-4 border-b border-white/[0.04] px-4 py-3 transition last:border-0 hover:bg-white/[0.03]"
						style="animation:slideRight 0.3s ease {i * 30}ms backwards"
					>
						<div class="flex items-center gap-2">
							<div
								class="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br {colors.from} {colors.to} text-xs font-bold text-white"
							>
								{market.underlying.slice(0, 2)}
							</div>
							<div>
		...
		<!-- ── FOOTER ── -->
		<div
			class="mt-12 flex flex-col items-center justify-between gap-4 border-t border-white/[0.06] pt-6 sm:flex-row"
		>
			<div class="flex items-center gap-2">
				<div class="relative">
